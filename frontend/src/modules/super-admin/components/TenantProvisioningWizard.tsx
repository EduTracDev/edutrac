...
export function TenantProvisioningWizard({ onClose, onComplete }: TenantProvisioningWizardProps) {
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 border-b border-slate-200 bg-slate-50 flex justify-between items-center">
          <div>
            <h2 className="text-lg font-bold text-slate-900">Provision New School Tenant</h2>
            <p className="text-xs text-slate-500">Set up multi-tenant workspace, subdomains, and primary admin access.</p>
          </div>
        ...
        <div className="px-6 py-4 bg-slate-100/60 border-b border-slate-200 grid grid-cols-3 gap-2 text-center text-xs">
          <div className={`py-2 rounded-lg font-bold transition-colors ${currentStep === 1 ? "bg-indigo-600 text-white" : "bg-slate-200 text-slate-600"}`}>
            1. School Identity
          </div>
          <div className={`py-2 rounded-lg font-bold transition-colors ${currentStep === 2 ? "bg-indigo-600 text-white" : "bg-slate-200 text-slate-600"}`}>
            2. Resource Tier
          </div>
          <div className={`py-2 rounded-lg font-bold transition-colors ${currentStep === 3 ? "bg-indigo-600 text-white" : "bg-slate-200 text-slate-600"}`}>
            3. Primary Admin
          </div>
        </div>
        ...
        <div className="p-6 flex-1 overflow-y-auto space-y-4 text-xs">
          ...
          {currentStep === 1 && (
            <div className="space-y-4">
              <div>
                <label className="block font-bold text-slate-700 mb-1">School / Organization Name</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-1">Workspace Subdomain URL</label>
                <div className="flex items-center">
                  <input
                    type="text"
                    value={step1.subdomain}
                    onChange={(e) => setStep1({ ...step1, subdomain: e.target.value.toLowerCase() })}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setStep1({ ...step1, contactEmail: e.target.value })}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setStep1({ ...step1, contactPhone: e.target.value })}
          ...
          {currentStep === 2 && (
            <div className="space-y-4">
              <div>
                <label className="block font-bold text-slate-700 mb-2">Select Subscription Tier</label>
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { tier: "Basic", desc: "Up to 500 Students", price: "$150/mo" },
                    { tier: "Professional", desc: "Up to 2,000 Students", price: "$350/mo" },
                    { tier: "Enterprise", desc: "Unlimited + Dedicated SLA", price: "$600/mo" },
                  ].map((item) => (
                    <div
                      key={item.tier}
                      onClick={() => setStep2({ ...step2, planTier: item.tier as any })}
                    ...
                    >
                      ...
                      <p className="text-xs font-extrabold text-indigo-600 mt-3">{item.price}</p>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div className="flex gap-4">
                  <label className="flex items-center gap-2 cursor-pointer font-medium text-slate-700">
                    <input
                      ...
                      onChange={() => setStep2({ ...step2, billingCycle: "Annual" })}
                  ...
                  <label className="flex items-center gap-2 cursor-pointer font-medium text-slate-700">
                    <input
                      ...
                      onChange={() => setStep2({ ...step2, billingCycle: "Monthly" })}
                      className="text-indigo-600 focus:ring-indigo-500"
                    />
                    Monthly Billing
                  </label>
          ...
          {currentStep === 3 && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Admin First Name</label>
                  <input
                    type="text"
                    value={step3.adminFirstName}
                    onChange={(e) => setStep3({ ...step3, adminFirstName: e.target.value })}
                    placeholder="David"
                    className="w-full p-2.5 border border-slate-300 rounded-lg outline-none"
                  />
                </div>
                <div>
                  <label className="block font-bold text-slate-700 mb-1">Admin Last Name</label>
                  <input
                    type="text"
                    value={step3.adminLastName}
                    onChange={(e) => setStep3({ ...step3, adminLastName: e.target.value })}
                    placeholder="Miller"
                    className="w-full p-2.5 border border-slate-300 rounded-lg outline-none"
                  />
                </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setStep3({ ...step3, adminEmail: e.target.value })}
              ...
              <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl space-y-2">
                <label className="flex items-center gap-2 font-bold text-amber-900 cursor-pointer">
                  <input
                    ...
                    onChange={(e) => setStep3({ ...step3, sendWelcomeEmail: e.target.checked })}
                    className="rounded text-amber-600 focus:ring-amber-500"
                  />
                  Send Activation & Password Setup Email
                </label>
                <p className="text-[11px] text-amber-800">
                  An automated setup link will be emailed to the primary admin to finalize their password setup and onboarding steps.
                </p>
              </div>
            </div>
          )}
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50 flex justify-between items-center">
          ...
          {currentStep < 3 ? (
            ...
            >
              {isDeploying && <span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />}
              {isDeploying ? "Provisioning Tenant..." : "Deploy School Workspace"}
            </button>
          )}
        </div>
      </div>

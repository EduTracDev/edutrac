...
import { MOCK_TICKETS, SupportTicket, TicketStatus, TicketPriority } from "@/modules/super-admin/types/support.types";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Support & Issue Resolution</h1>
          <p className="text-sm text-slate-500">
            Manage incoming inquiries, technical bug reports, and billing issues from school administrators.
          </p>
        </div>
      </div>
          ...
          {["All", "Open", "In Progress", "Resolved", "Closed"].map((st) => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors ${
                activeStatus === st ? "bg-slate-900 text-white" : "text-slate-600 hover:bg-slate-100"
              }`}
            >
              {st}
            </button>
        ...
        <div className="flex items-center gap-2">
          ...
          {["All", "Urgent", "High", "Medium", "Low"].map((pr) => (
            <button
              ...
              className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors ${
                activePriority === pr ? "bg-indigo-600 text-white" : "text-slate-600 hover:bg-slate-100"
              }`}
            >
              {pr}
            </button>
              ...
              {filteredTickets.length > 0 ? (
                filteredTickets.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="p-3 font-bold text-slate-900">{t.schoolName}</td>
                    <td className="p-3 font-medium text-slate-800 max-w-xs truncate">{t.subject}</td>
                    <td className="p-3">

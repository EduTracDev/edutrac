// frontend/src/app/(super-admin)/super-admin/subscriptions/page.tsx
"use client";
...
export default function SubscriptionsPage() {
  const [subscriptions, setSubscriptions] = useState<TenantSubscription[]>(MOCK_SUBSCRIPTIONS);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Subscriptions & SaaS Invoicing</h1>
          <p className="text-sm text-slate-500">
            Monitor tenant licensing, process tier upgrades, and track revenue collection across all schools.
          </p>
        </div>
        {activeTab === "invoices" && (
          <button
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {subscriptions.map((sub) => (
                  <tr key={sub.id} className="hover:bg-slate-50 transition-colors">
                    <td className="p-3 font-bold text-slate-900">{sub.schoolName}</td>
                    <td className="p-3">
                    ...
                    <td className="p-3 font-semibold text-slate-900">${sub.amount.toLocaleString()}</td>
                    <td className="p-3 text-slate-500">{sub.nextBillingDate}</td>
                    <td className="p-3">
                      <span
              ...
              <tbody className="divide-y divide-slate-100 text-slate-700">
                {invoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-3 font-bold text-slate-900">{inv.schoolName}</td>
                    <td className="p-3">{inv.planTier}</td>
                    <td className="p-3 font-semibold text-slate-900">${inv.amount.toLocaleString()}</td>

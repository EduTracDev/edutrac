...
export default function SchoolManagementPage() {
  ...
  const handleFormSubmit = async (formData: SchoolFormData) => {
    ...
    if (selectedSchool) {
      ...
      setSchools((prev) => prev.map((s) => (s.code === formData.code ? formData : s)));
    } else {
      // Create new record
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-slate-500">Register, inspect, and configure schools across the system.</p>
        </div>
        <button
            ...
            {schools.map((school) => (
              <tr key={school.code} className="hover:bg-slate-50">
                <td className="p-4">
                  ...
                  <p className="text-xs text-slate-400">{school.code} • {school.email}</p>
                </td>
                <td className="p-4">
                ...
                <td className="p-4">
                  ...
                  <p className="text-xs text-slate-400 mt-1">Cap: {school.maxStudents} students</p>
                </td>
                <td className="p-4">
                  <span

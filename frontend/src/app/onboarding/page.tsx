...
import { Plus, X, ArrowLeft, ArrowRight, UploadCloud, Layout, CheckCircle, Video, Users, UserPlus, Network, Calendar, Smartphone, Tablet, Monitor } from "lucide-react";
...
export default function OnboardingComponent() {
  ...
  const handleForward = () => { if (currentStep < totalSteps) setCurrentStep(prev => prev + 1); };
  const handleBackward = () => { if (currentStep > 1) setCurrentStep(prev => prev - 1); };

  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, setTarget: (val: string | null) => void, isLogo: boolean = false) => {
      ...
      <main className="w-full max-w-6xl mx-auto px-4 py-8 flex-grow flex flex-col justify-center items-center">
        ...
        {currentStep === 1 && (
          <div className="w-full max-w-xl space-y-6 animate-in fade-in duration-200">
            <div className="text-center">
              <h1 className="text-3xl font-black text-[#1E1E2F] tracking-tight">Add account details</h1>
              <p className="text-sm text-gray-400 mt-1">We&apos;ve gotten you started</p>
            </div>
            ...
            <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-xs space-y-6">
                ...
                {PRESET_COLORS.map((color) => (
                  <button
                    ...
                    className={`w-11 h-11 rounded-full transition-all ${
                      selectedColor === color ? "ring-4 ring-offset-2 ring-dynamicBrand scale-105" : "border border-black/5 hover:scale-105"
                    }`}
                  />
                ))}
                <button onClick={() => setShowColorPicker(true)} className="w-11 h-11 rounded-full bg-gray-50 border border-gray-200 flex items-center justify-center text-gray-500 hover:bg-gray-100 transition-colors">
                  <Plus size={18} />
                </button>
              </div>
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="space-y-1">
                  ...
                  <input type="file" ref={banner1Ref} className="hidden" accept="image/*" onChange={(e) => handleFileChange(e, setBanner1)} />
                </div>
              ...
              <div className="pt-2 text-center">
                <button type="button" onClick={() => setShowLogoModal(true)} className="text-xs font-bold text-dynamicBrand hover:underline">
        ...
        {currentStep === 2 && (
          <div className="w-full max-w-4xl space-y-8 animate-in fade-in duration-200">
            <div>
              <h1 className="text-3xl font-black text-[#1E1E2F] tracking-tight">Setup Options</h1>
              <p className="text-xs text-gray-400 mt-1">Choose the setup options that best fit your school type.</p>
            </div>

            <div className="space-y-6">
              <div>
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Physical School Matrix</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    { icon: <Calendar size={18} />, title: "Academic Calendar", desc: "Define fields for terms, academic years, and vacation slots." },
                    { icon: <Network size={18} />, title: "Campus Branches", desc: "Organize records smoothly across multiple territorial locations." },
                    { icon: <Users size={18} />, title: "Staff Directory", desc: "Provision profiles for active educators and department directors." },
                    { icon: <UserPlus size={18} />, title: "Parent Network", desc: "Invite family guardians to track pupil grades and schedules." },
                  ].map((item, idx) => (
                    <div key={idx} className="bg-white border border-gray-200 rounded-xl p-5 hover:border-dynamicBrand transition-all group cursor-pointer">
                      ...
                      <h4 className="text-xs font-bold text-[#1E1E2F] mb-1">{item.title}</h4>
        ...
        {currentStep === 3 && (
          <div className="w-full max-w-xl text-center space-y-6 animate-in fade-in duration-200">
            <h1 className="text-4xl font-black text-[#1E1E2F] tracking-tight leading-tight">Let&apos;s create your school&apos;s landing page</h1>
            ...
            <div className="pt-2">
              <button onClick={handleForward} className="px-8 py-3 bg-dynamicBrand hover:bg-dynamicBrandHover text-white text-xs font-bold rounded-xl shadow-xs transition-colors">
                Start Setup Wizard
              </button>
            </div>
          </div>
          ...
          <div className="w-full grid grid-cols-1 lg:grid-cols-12 gap-8 items-start animate-in fade-in duration-300">
            <div className="lg:col-span-4 bg-white border border-gray-100 rounded-2xl p-5 space-y-5 shadow-xs">
              ...
              <div className="space-y-1">
                ...
                <input type="text" value={heroTitle} onChange={(e) => setHeroTitle(e.target.value)} className="w-full px-3 py-2 border border-gray-200 rounded-xl text-xs focus:outline-none focus:border-dynamicBrand" />
              </div>

              <div className="space-y-1">
                <label className="text-[11px] font-bold text-gray-500">Banner Subtitle Description</label>
                <textarea rows={3} value={heroSubtitle} onChange={(e) => setHeroSubtitle(e.target.value)} className="w-full px-3 py-2 border border-gray-200 rounded-xl text-xs focus:outline-none focus:border-dynamicBrand resize-none" />
              </div>
            </div>
            ...
            <div className="lg:col-span-8 space-y-4">
              <div className="flex justify-center items-center gap-2 bg-white p-1.5 rounded-xl border border-gray-200 w-fit mx-auto shadow-2xs">
                {["desktop", "tablet", "mobile"].map((mode) => (
                  <button key={mode} onClick={() => setViewMode(mode as any)} className={`px-3 py-1.5 rounded-lg text-xs font-bold capitalize transition-all ${viewMode === mode ? "bg-gray-100 text-[#1E1E2F]" : "text-gray-400"}`}>
              ...
              <div className="w-full flex justify-center transition-all duration-300">
                <div className={`bg-white border border-gray-200 rounded-2xl shadow-xl overflow-hidden transition-all duration-300 ${viewMode === "desktop" ? "w-full" : viewMode === "tablet" ? "w-[500px]" : "w-[340px]"}`}>
                  ...
                  <div className="p-6 text-center space-y-4">
                    <h2 className="text-xl font-black text-gray-900">{heroTitle}</h2>
        ...
        {currentStep === 5 && (
          <div className="w-full max-w-xl space-y-6 animate-in fade-in duration-200">
            <div className="text-center">
              <h1 className="text-3xl font-black text-[#1E1E2F] tracking-tight">Add Media Gallery</h1>
              <p className="text-xs text-gray-400 mt-1">Upload viewable imagery assets of campus facilities or academic buildings.</p>
            </div>

            <div className="bg-white border border-gray-100 rounded-3xl p-6 shadow-xs space-y-4">
              <div onClick={() => mediaRef.current?.click()} className="border-2 border-dashed border-gray-200 hover:border-dynamicBrand rounded-2xl p-8 flex flex-col items-center justify-center bg-gray-50/50 cursor-pointer transition-all">
                <UploadCloud size={24} className="text-gray-400 mb-2" />
                <h4 className="text-xs font-bold text-gray-700">Browse Image Archives</h4>
              </div>
              <input type="file" ref={mediaRef} multiple className="hidden" accept="image/*" onChange={(e) => {
                if (e.target.files) {
                  const filesArr = Array.from(e.target.files).map(f => URL.createObjectURL(f));
              ...
              {mediaImages.length > 0 && (
                <div className="grid grid-cols-4 gap-2 pt-2">
                  {mediaImages.map((img, index) => (
                    <div key={index} className="aspect-square bg-gray-100 rounded-lg relative overflow-hidden border border-gray-200">
                      <Image src={img} alt="Uploaded Item" fill className="object-cover" />
                    </div>
                  ))}
                </div>
              )}
        ...
        {currentStep === 6 && (
          <div className="w-full max-w-xl text-center space-y-6 animate-in fade-in duration-200">
            ...
            <h1 className="text-2xl font-black text-[#1E1E2F] tracking-tight">Your configuration is locked!</h1>
            <p className="text-xs text-gray-400 max-w-xs mx-auto">Click confirm below to launch your corporate layout settings live into the workspace database layers.</p>
            <div className="pt-2">
              <button onClick={handleForward} className="px-8 py-2.5 bg-dynamicBrand hover:bg-dynamicBrandHover text-white text-xs font-bold rounded-xl shadow-xs transition-colors">
        ...
        {currentStep === 7 && (
          <div className="w-full max-w-md text-center space-y-4 animate-in zoom-in-95 duration-300">
            ...
            <h1 className="text-2xl font-black text-[#1E1E2F] tracking-tight">Onboarding Complete!</h1>
            <p className="text-xs text-gray-400 leading-relaxed">
              Your profile configurations are saved successfully. You are now being forwarded straight into the core school administration workspace dashboard.
            </p>
          </div>
        )}
      ...
      {showColorPicker && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-xs p-5 relative shadow-2xl animate-in zoom-in-95 duration-150">
            <button onClick={() => setShowColorPicker(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-700">
              <X size={16} />
            </button>
            <div style={{ backgroundColor: selectedColor }} className="w-full h-32 rounded-2xl mb-4" />
            <input type="range" min="0" max="360" className="w-full h-2 bg-linear-to-r from-red-500 via-green-500 to-blue-500 rounded-lg appearance-none cursor-pointer mb-4" onChange={(e) => applyThemeColor(`hsl(${e.target.value}, 65%, 55%)`)} />
      ...
      {showLogoModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 relative shadow-2xl">
            <button onClick={() => setShowLogoModal(false)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-700">
              <X size={18} />
            </button>
            <div className="text-center my-2">
              <h3 className="text-sm font-bold text-gray-900">Upload Institution Logo</h3>
            </div>
            <div onClick={() => logoRef.current?.click()} className="border-2 border-dashed border-gray-200 hover:border-dynamicBrand rounded-xl p-8 flex flex-col items-center justify-center bg-gray-50/50 cursor-pointer transition-all">
            ...
            <input ref={logoRef} type="file" accept="image/*" className="hidden" onChange={(e) => handleFileChange(e, setLogoName, true)} />
          </div>
        </div>
      )}

...
import { INVITATION_EMAIL_CONFIG } from '../config/invitaion-email-config';
...
export class InvitationService {
    ...
    async inviteUser(dto: CreateUserInvitationDto, tenantId: number) {
    ...
    async acceptInvitationViaGoogle(dto: AcceptUserInvitationGoogleDto){
        const tokenHash = this.tokenService.hashToken(dto.token);
        const invitation = await this.prismaService.invitation.findUnique({
            where: {
                tokenHash
        ...
        if(!googleUser || !googleUser.email) throw new BadRequestException('Invalid token');
        if(invitation.email !== googleUser.email) throw new ForbiddenException('Invalid request. Suspicious activity detected');
        const user = await this.prismaService.user.findUnique({
            where: {
                email_tenantId: {
                    email: invitation.email,
        ...
        if(!user) throw new NotFoundException('This account does not exist. Kindly contact the administrator for assistance');
        const profileData = {
    ...
    async resendInvitation(dto: ResendInvitationDto){
        ...
        if (!invitation) throw new NotFoundException('Invitation not found');
        if (invitation.status === 'ACCEPTED') throw new BadRequestException('This invitation has already been accepted');
        if (invitation.status !== 'PENDING') throw new BadRequestException('Invalid or expired invalid');
        const token = this.tokenService.generateToken();
        const tokenHash = this.tokenService.hashToken(token);
